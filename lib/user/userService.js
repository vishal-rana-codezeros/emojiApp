...
function editProfile(req, res) {
	...
	return emoji_schema.findOne({ "emailId": req.body.emailId }).then(async (result) => {
		if (result) {
			return userMapper.editProfileEmailCheck(result).then((resp) => {
			...
			return emoji_schema.findOne({ $and: [{ "contactNumber": req.body.contactNumber }, { "isdCode": req.body.isdCode }] }).then(async (result) => {
				...
				if (result) {
					return userMapper.editProfileNumberCheck(result).then((resp) => {
						return resp
					}).catch(err => { return { status: false, statusCode: userConst.CODES.INTRNLSRVRERR, message: userConst.MESSAGES.internalServerError } })
				}
					...
					return userDao.updateData(id, obj).then((result) => {
						if (result) {
							return userMapper.updateProfileSuccess(result).then((resp) => {
								// var Password = securePin.generatePinSync(4);
								// let html = `Verify Your Account Using This OTP :-</b><h1>${Password}</h1>`
								// util.sendMail(result.emailId, userConst.MAILOBJ.regiterMailSubject, html)
								// newPin = bcrypt.hashSync(Password, 10);
								...
								// 		}).catch(err => { return { stats: false, statusCode: userConst.CODES.FAILURECASE, message: userConst.MESSAGES.internalServerError } })
								// 	}
								// 	else {
								// 		return { status: false, staatusCode: userConst.CODES.FAILURECASE, message: userConst.MESSAGES.incorrectid }
								// 	}
								// }).catch(err => { return ({ statu: false, statusCode: userConst.CODES.FAILURECASE, message: userConst.MESSAGES.internalServerError }) })
								return resp
...
function addcontactUs(req, res) {
	return userMapper.allowContactUsFields(req).then(response => {
		...
		}).catch(err => { return { stat: false, statusCode: userConst.CODES.INTRNLSRVRERR, message: userConst.MESSAGES.internalServerError } })
	}).catch(err => { return { statu: false, statusCode: userConst.CODES.INTRNLSRVRERR, message: userConst.MESSAGES.internalServerError } })
}
//<--------------------------------------------------------------------Updated Api`s ended------------------------------------------------------------------------------------------------------------------------------------------------------------>
