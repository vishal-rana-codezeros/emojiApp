...
async function sendMessage1(req, res) {

	console.log("resp from api", req.body)


	let msgBody = `follow this invitation link to use EmojiApp: - `;

	await util.sendMessage(msgBody, req.body)
...
function verifyAccount(req, res) {
	...
	}).catch(err => { res.json({ code: userConst.CODES.NOTFOUND, message: userConst.MESSAGES.incorrectid }) })
}
...
function resendPin(req, res) {
	...
	emoji_schema.findOne({ "_id": req.params.id }).then(async (data) => {
		if (data) {
			...
			return res.json({ code: userConst.CODES.NOTFOUND, message: userConst.MESSAGES.incorrectid })
		}
	}).catch(err => { res.json({ code: userConst.CODES.NOTFOUND, message: userConst.MESSAGES.incorrectid }) })
}
		...
		if (err) {
			...
			if (data.role == 'ADMIN') {
				...
				if (data.isVerify) {
					...
					return res.json({ code: userConst.CODES.FORBIDDEN, message: userConst.MESSAGES.errorInVerifyAccount })
				}
			}
		}
...
function forgotPassword(req, res) {
	emoji_schema.findOne({ "emailId": req.body.emailId }, async (err, data) => {
		if (err) {
			...
			return res.json({ code: userConst.CODES.BADREQUEST, message: userConst.MESSAGES.incorrectid })
	...
	emoji_schema.findOne({ "_id": req.params.id }).then((result) => {
		{
			var match = bcrypt.compare(req.body.oldPassword, result.password).then((result1) => {
				if (result1 == true) {
					...
					return res.json({ code: userConst.CODES.BADREQUEST, message: userConst.MESSAGES.invalOldPwd })
				}
			})
		}
	}).catch(err => { res.json({ code: userConst.CODES.NOTFOUND, message: userConst.MESSAGES.incorrectid }) })
...
function getUserDetails(req, res) {
	emoji_schema.find({ role: "USER", _id: { $exists: true, $ne: [req.params.id] } }, (err, data) => {
		if (err) {
			return res.json({ code: userConst.CODES.INTRNLSRVRERR, message: userConst.MESSAGES.internalServerError });
		}
		else {
