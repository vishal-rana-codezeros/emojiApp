...
async function register(req, res) {
	...
	return emoji_schema.findOne({ $or: [{ "emailId": req.body.emailId }, { "contactNumber": req.body.contactNumber }] }).then(async (result) => {
		...
		if (result) {
			return { code: userConst.CODES.FORBIDDEN, message: userConst.MESSAGES.userExists }
		}
		else {
			...
			return userDao.registerUser(obj).then(async (data) => {
				...
				util.sendMail(req.body.emailId, userConst.MAILOBJ.regiterMailSubject, html);

				return { code: userConst.CODES.SUCCESS, message: userConst.MESSAGES.registerSuccess }

			}).catch(err => {
				console.log(">>>>>>>>>>",err);return { code: userConst.CODES.INTRNLSRVRERR, message: userConst.MESSAGES.registerFailure }
			})
		}
...
async function login(req, res) {
	...
	return userDao.getUser(id).then(async (data) => {
		if (data) {
			...
			return { responseCode: userConst.CODES.BADREQUEST, responseMessage: userConst.MESSAGES.incorrectEmailid }
		}
	}).catch(err => { return { code: userConst.CODES.INTRNLSRVRERR, message: userConst.MESSAGES.internalServerError } })
}
function forgotPassword(req, res) {
	...
	return userDao.getUser(id).then(async (data) => {
		if (!data) {
			...
			console.log("into forgot pasword function ", data)
...
// 	return userDao.socialId(id).then(async (data) => {
...
// 					var Password = securePin.generatePinSync(6);
// 					let html = `We received a request to reset your EmojiApp password. Your new password is  :-</b><h1>${Password}</h1>`
// 					await util.sendMail(data.emailId, userConst.MAILOBJ.forgotPasswordMailSubject, html).then((result) => {
// 						if (result) {
// 							data.password = bcrypt.hashSync(Password, 10);
// 							let id = req.body.emailId
// 							let obj = {}
...
// 						}
// 						else {
// 							return ({ code: userConst.CODES.INTRNLSRVRERR, message: userConst.MESSAGES.mailFailure })
// 						}
// 					}).catch(err => { return ({ code: userConst.CODES.INTRNLSRVRERR, message: userConst.MESSAGES.internalServerError }) })
// 				}
// 			}).catch(err => { return ({ code: userConst.CODES.INTRNLSRVRERR, message: userConst.MESSAGES.internalServerError }) })
...
// 		else {
// 			let obj = req.body
// 			return userDao.registerUser(obj).then((body) => {
// 				if (body) {
// 					return ({ code: userConst.CODES.SUCCESS, message: userConst.MESSAGES.registerSuccess })
// 				}
// 				else {
...
// }
function socialLogin(req, res) {
	console.log('into socialLogin')
	var pin = securePin.generatePinSync(4);
	let id = req.body.socialId
	return userDao.socialId(id).then((data) => {

		if (!data) {
			console.log("into if no data")
			let obj = req.body;

			return userDao.registerUser(obj).then(async (data1) => {
				console.log("into if no data1", data1)
			...
			console.log("into else part", data)
			return jwtHandler.genUsrToken({ id: data._id }).then((jwt) => {
					...
					await cloudinary.v2.uploader.upload(`${process.cwd()}/img/${filePath}`, async (error, result) => {
						if (result) {
							try {
								...
								require('fs').unlink(`${process.cwd()}/img/${filePath}`, (error_res, res_data) => {
									return error_res ? error_res : res_data
								});
							} catch (e) {
...
async function getUserDetails(req, res) {
	return emoji_schema.findOne({ role: "USER", _id: { $exists: true, $ne: [req.params.id] } }).then((data) => {
		if (!data) {
			return { code: userConst.CODES.NOTFOUND, message: userConst.MESSAGES.noDataFound, result: data }
		}
		else {
			return { code: userConst.CODES.SUCCESS, message: userConst.MESSAGES.userDetails, result: data }
		}
...
function getChatDetails(req, res) {
	messageSchema.find({ roomId: req.params.id }).populate('senderId', ['userName']).exec(function (err, data) {
		if (err) {
			...
			return { code: userConst.CODES.SUCCESS, message: userConst.MESSAGES.userDetails, data: data }
		}
	})
}
