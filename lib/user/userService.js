...
async function syncContacts(req, res) {
	...
	data.contacts.forEach((val, idx) => {
		let contactIdx = result_from_data.findIndex((selectedContact) => (val.contactNumber == (selectedContact.isdCode.concat(selectedContact.contactNumber)) || val.contactNumber == selectedContact.contactNumber))
		if (contactIdx >= 0) {
			//found contact info
