...
async function getClientTokenForPayment(res, userId, keyboardId) {
	...
	if (!userData[1]) {
		return res.json({ code: userConst.CODES.NOTFOUND, message: userConst.MESSAGES.NOKEYBOARDFOUND })
	} else if (!userData[0]) {
		return res.json({ code: userConst.CODES.NOTFOUND, message: userConst.MESSAGES.noDataFound })
	} else {
		...
		return res.json({
			...
			message: userConst.MESSAGES.success,
			data: {
				token: generateToken,
...
async function completeTransaction(res, userId, transactionId, nonce) {
	...
	if (!userData[1]) {
		return res.json({ code: userConst.CODES.NOTFOUND, message: userConst.MESSAGES.NOTRANSACTIONFOUND })
	} else if (!userData[0]) {
		return res.json({ code: userConst.CODES.NOTFOUND, message: userConst.MESSAGES.noDataFound })
	} else {
		...
		if (resultFromNonce.success) {
			...
			if (!result_v2[0]){
				return res.json({ code: userConst.CODES.INTRNLSRVRERR, message: userConst.MESSAGES.KEYBOARDBOUGHTERROR })
			}

			if (result_v2[0] && result_v2[1]) {
				return res.json({ code: userConst.CODES.SUCCESS, message: userConst.MESSAGES.success })
			}
		} else {
			obj["paymentStatus"] = constant.STATUS.REJECTED;
			userDao.updateTransaction(transactionId, obj).then(result_v1 => {
				return res.json({ code: userConst.CODES.SUCCESS, message: userConst.MESSAGES.PAYMENTREJECTED })
			})
		}
	}
}
