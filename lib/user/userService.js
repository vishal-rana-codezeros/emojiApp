...
/**
 * this api is used for  fetch purchased keyboard by user.
 */
function fetchPurchasedKeyboard(req, res) {
	return emoji_schema.findOne({ "_id": req.params.id }).populate('subscription.id').then((data) => {
		if (data) {
			let filteredValue = data.subscription.filter(val => {
				if (new Date(val.expiry).getTime() > new Date().getTime()) {
					return val;
				}
			})
			return { status: true, statusCode: userConst.CODES.SUCCESSCASE, message: userConst.MESSAGES.success, data: filteredValue }
...
async function checkEmailExist(req, res) {
	...
	return emoji_schema.findOne({ "emailId": req.body.emailId, status: "ACTIVE" }).then(async (result) => {
		...
		if (result) {
			...
			return { status: true, statusCode: userConst.CODES.SUCCESSCASE, message: userConst.MESSAGES.userNotExists, data: { emailExits: false, pin: emailOTP } }
		}
...
async function checkNumberExist(req, res) {
	...
	return emoji_schema.findOne({ $and: [{ "contactNumber": req.body.contactNumber }, { "isdCode": req.body.isdCode }] }).then(async (result) => {
		...
		if (result) {
			...
			return { status: true, statusCode: userConst.CODES.SUCCESSCASE, message: userConst.MESSAGES.numNotExits, data: { numberExits: false, pin: smsPin } }
		}
...
function purchaseKeyboard(req, res) {
	...
	return emoji_schema.findOne({ "_id": req.params.id }).then(async (data) => {
		...
		if (data != null) {
			...
			let timeStamp = new Date().getTime() + 31536000000;

			return emoji_schema.findOneAndUpdate({ $and: [{ _id: data._id }, { 'subscription.id': { $ne: req.body.keyboardName } }] }, { $push: { "subscription": { purchaseAt: nowDate, expiry: timeStamp, id: req.body.keyboardName } } }, { new: true }).populate('subscription').then(async (data) => {
...
function getKeyboardFromCategory(req, res) {
	...
	return keyboardSchemas.find({ "categoryName": mongoose.Types.ObjectId(req.params.id) }).then((data) => {
...
function fetchKeyboardSubImages(req, res) {
	return keyboardSchemas.findById({ "_id": req.params.id }).then((data) => {
		...
		return { status: true, statusCode: userConst.CODES.SUCCESSCASE, message: userConst.MESSAGES.success, data: data1 }
...
//////////////////////////////////////////////// KeyBoard  transaction section ///////////////////////////////////////////

/**
 * this function will generate a token for the client that can be used to initiate the transaction
 * @param {*} res, the current response object
 * @param {*} userId, id of the current login user 
 * @param {*} keyboardId, id of the keyboard on which the action needs to be performed. 
 */

async function getClientTokenForPayment(res, userId, keyboardId) {
	let userData = await Promise.all([userDao.getUserData(userId).then(result => result), userDao.getImage(keyboardId).then(result_k => result_k)]);
	if (!userData[1]) {
		return res.json(userMapper.successResponse(userConst.CODES.FAILURECASE, userConst.CODES.NOTFOUND, userConst.MESSAGES.NOTFOUND))
	} else if (!userData[0]) {
		return res.json(userMapper.successResponse(userConst.CODES.FAILURECASE, userConst.CODES.NOTFOUND, userConst.MESSAGES.noDataFound))
	} else {
		//has value for user and keyboard
		let obj = {
			userId: userId,
			price: userData[1].cost,
			keyBoardId: userData[1]._id,
			keyBoardType: userData[1].keyboardType,
		};
		let generateToken = await brainTree.getTokenForClient()
		let createTransaction = await userDao.createTransaction(obj);
		return res.json(userMapper.successResponse(userConst.CODES.SUCCESSCASE, userConst.CODES.SUCCESS, userConst.MESSAGES.success, {
			token: generateToken,
...
/**
 * this function will complete the transaction pending and add the keyboard to user profile
 * @param {*} res, the current response object
 * @param {*} userId,  id of the current login user 
 * @param {*} transactionId , id of the ongoing transaction
 * @param {*} nonce , nonce generated from the transaction that was initialilzed from the client token
 */
async function completeTransaction(res, userId, transactionId, nonce) {
	let userData = await Promise.all([userDao.getUserData(userId).then(result => result), userDao.fetchTransactionFromId(transactionId).then(result_k => result_k)]);
	if (!userData[1]) {
		return res.json(userMapper.successResponse(userConst.CODES.FAILURECASE, userConst.CODES.NOTFOUND, userConst.MESSAGES.NOTRANSACTIONFOUND))
	} else if (!userData[0]) {
		return res.json(userMapper.successResponse(userConst.CODES.FAILURECASE, userConst.CODES.NOTFOUND, userConst.MESSAGES.noDataFound))
	} else {
		let resultFromNonce = await brainTree.validateNonceAndInitiatePayment(nonce, userData[1].price);
		console.log(resultFromNonce.success)
		let obj = {};
		if (resultFromNonce.success) {
			obj["paymentStatus"] = constant.STATUS.COMPLETED;
			obj["transactionNonce"] = nonce;
			let nowDate = Date.now();
			let timeStamp = new Date().getTime() + 31536000000;
			let result_v2 = await Promise.all([
					...
					$and: [
						...
						$push: {
							"subscription": {
								purchaseAt: nowDate,
								expiry: timeStamp,
								id: userData[1].keyBoardId
							}
						}
					}, { new: true }).populate('subscription'),
				userDao.updateTransaction(transactionId, obj)
			])
			if (!result_v2[0]) {
				return res.json(userMapper.successResponse(userConst.CODES.FAILURECASE, userConst.CODES.INTRNLSRVRERR, userConst.MESSAGES.KEYBOARDBOUGHTERROR))
			}

			if (result_v2[0] && result_v2[1]) {
				return res.json(userMapper.successResponse(userConst.CODES.SUCCESSCASE, userConst.CODES.SUCCESS, userConst.MESSAGES.success))
			}
		} else {
			obj["paymentStatus"] = constant.STATUS.REJECTED;
			userDao.updateTransaction(transactionId, obj).then(result_v1 => {
				return res.json(userMapper.successResponse(userConst.CODES.FAILURECASE, userConst.CODES.INTRNLSRVRERR, userConst.MESSAGES.PAYMENTREJECTED))
			})
		}
	}
}
...
module.exports =
	{
		...
		getAllCategory,
		getAllKeyboard,
		fetchPurchasedKeyboard,
		getKeyboardFromCategory,

		getClientTokenForPayment, //creating a client token for payment

		completeTransaction, //complete the process of previously inititated transaction
