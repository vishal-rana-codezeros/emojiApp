...
/**
 * this route use to fetch purchased keyboard by user.
 */
userRouter.route("/fetchPurchasedKeyboard/:id")
    .get([upload.none()], (req, res) => {
        userService.fetchPurchasedKeyboard(req, res).then((data) => {
            return res.json(data)
        })
    })
...
userRouter.route("/getKeyboardFromCategory/:id")
    .get([validate.verifyToken], (req, res) => {
        userService.getKeyboardFromCategory(req, res)
    })
//<---------------------------------------------------------------Updated Api`s ended--------------------------------------------------------------------------------------------------------------------------------->
...
userRouter.route("/verifyAccount/:id")
    .post([upload.none(), validate.validateverifyAccount], (req, res) => {
...
///////////////////////////////////// Keyboard specific tasks /////////////////////////////////////////////////



userRouter.get("/getTransactionToken/:userId/:keyboardId", [validate.verifyToken], (req, res) => {
    let { userId, keyboardId } = req.params;
    userService.getClientTokenForPayment(res, userId, keyboardId);
})


userRouter.post("/completeTransaction/:userId/:transactionId", [validate.verifyToken, upload.none()], (req, res) => {
    let { userId, transactionId } = req.params;
    let { nonce } = req.body;
    userService.completeTransaction(res, userId, transactionId, nonce);
})
