...
function getAllKeyboardDetails(req, res) {
	...
	keyboard_Schema.paginate(query, options, (err, data) => {
...
function getOneCategoryData(req, res) {
	let options = {}
	category.findOne({ "_id": req.params.id }, '_id status category createdBy', (err, data) => {
		if (err) {
			return res.json({ code: adminConst.CODE.internalServerError, message: adminConst.MESSAGE.internalServerError })
		}
		else {
			return res.json({ code: adminConst.CODE.SUCCESS, message: adminConst.MESSAGE.getUserData, data: data })
		}
...
function getAllCategory(req, res) {
	...
	options.select = '_id status category createdBy'
