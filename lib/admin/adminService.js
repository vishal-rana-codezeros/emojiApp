...
function getAllKeyboardDetails(req, res) {
	...
	keyboard_Schema.aggregate([
		...
		, {
			...
			return res.json({ code: adminConst.CODE.SUCCESS, message: adminConst.MESSAGE.getUserData, data: data[0] })
		})
}
...
// 		}, {
// 			$addFields: {
// 				"data.categoryName": "$data.categoryDetails.category",
// 			}
// 		}
// 		, {
// 			$project: {
...
// 			}
// 		}]).then(data => {
// 			return res.json({ code: adminConst.CODE.INTERNALSERVER, message: adminConst.MESSAGE.noRecordsFound, data: data[0] })
// 		})
// }
...
function getAllCategory(req, res) {
	...
	category.paginate(query, options, (err, data) => {
		if (err) {
			...
			return res.json({ code: adminConst.CODE.INTERNALSERVER, message: adminConst.MESSAGE.noDataFound })
		}
		else {
			console.log(",,,,", data.docs.length)
			return res.json({ code: adminConst.CODE.SUCCESS, message: adminConst.MESSAGE.getUserData, data: data })
