...
function addKeyboard(req, res) {
	keyboard_Schema.findOne({ "keyboardName": req.body.keyboardName }).then(async (result) => {
		if (result) {
			...
			else if (req.body.keyboardType == 'free' && (req.body.cost > 0)) {
...
function updateKeyboardDetails(req, res) {
	...
	if (req.body.keyboardName) {
		...
		keyboard_Schema.findOne({ "keyboardName": req.body.keyboardName }, (err, data) => {
			if (data._id == req.params.id) {
				...
				return res.json({ code: adminConst.CODE.SUCCESS, message: "ALready exixts" })
			}
