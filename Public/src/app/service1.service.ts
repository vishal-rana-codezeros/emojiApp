...
export class Service1Service {
  ...
  register(user1: any) {
    return this.http.post("http://localhost:8080/register", JSON.stringify(user1), httpOptions)
  }


  login(user2: any) {

    return this.http.post("http://localhost:8080/login", JSON.stringify(user2), httpOptions)
  }
  ...
  registerdata() {
    ...
    return this.http.get("http://localhost:8080/getUserDetails/" + id, httpOptions)
  }

  getchat(roomId) {

    return this.http.get("http://localhost:8080/getChatDetails/" + roomId, httpOptions)

  }

  getgroup() {
    let id = localStorage.getItem('myId')
    return this.http.get("http://localhost:8080/groupData/" + id, httpOptions)
  }
